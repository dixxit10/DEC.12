/* ============================================================
   DEC.12 ・ app.js 檔案導覽（v38 方案 A 加註）
   ------------------------------------------------------------
   本檔為 minified（壓縮）單行檔，約 68KB / 115 個函式。
   依「實體出現順序」分區（括號內為字母代號，對應 docs/APP_STRUCTURE.md）：

   [A] 易經核心     hexFromTitle / HEX_TABLE / changedHex /
                    zhiGuaObject / resolveInterpretation /
                    readingLineText / readingFocusText / readingGuideText
   [C] 畫面切換     go / resetForm / setupChips / updateNav /
                    updateMenuAuth / doLogout / navLoginClick
   [D] 每日抽卡     dailyDraw（抽卡 + 翻牌）
   [E] 進階卜卦     tossOneLine / castSixLines / pickResult /
                    triToName / shuffle / startDivination /
                    stopDivination / renderCarousel / updateDots /
                    updateSaveBtn / safeRenderDiary / saveFlow
   [F] 帳號系統     setAuthMode / authFail / isValidEmail /
                    isStrongPassword / doLogin / openForgot /
                    closeForgot / showLoading / settings 系列
   [G] 日記系統     seedDiary / loadDiary / saveDiary / commitRecord /
                    mergeDraftsOnLogin / renderCalendar /
                    scrollCalToCurrent / addMonths / updateCalTitle
   [H] 收藏守護     collectKey / getCollectedElements / renderCollect /
                    unlockElementsFromHex / migrateGuestCollection /
                    guardian* / openGuardian / openDetail（詳情編輯）
   [I] 分享         shareText / shareUrl / openShare / shareViaApp
   [B] 語言系統     I18N / t / pick / lang / detectLang / setLang /
                    syncLangBtns / applyLangTexts / initLang /
                    syncLangUI / toggleLang

   修改守則：
   1. 改哪區就搜該區關鍵字（grep），勿動其他區
   2. 永不直接改壓縮行——先 beautify 定位，再改，再壓縮
   3. 新文案一律進 I18N 字典（中英都要），不 hardcode
   4. 改完執行 node --check + 手機實測該功能
   5. 完整守則見 docs/APP_STRUCTURE.md
   ============================================================ */
(function () {
  "use strict";
  var HEXAGRAMS = (typeof window != "undefined" && window.HEXAGRAMS) || [],
    ...
    getHexagrams = function () {
      ...
      return UI_LANG === "en" && en && en.length
        ? en
        : (typeof window != "undefined" && window.HEXAGRAMS) || [];
    },
    getLines = function () {
      var en = typeof window != "undefined" && window.LINES_EN;
      return UI_LANG === "en" && en
        ? en
        : (typeof window != "undefined" && window.LINES) || {};
    },
    TRI = {
      天: [1, 1, 1],
      澤: [1, 1, 0],
  ...
  for (var HEXAGRAM_IMG = {}, _hi = 1; _hi <= 64; _hi++)
    HEXAGRAM_IMG[_hi] = "img/hexagram-" + pad2(_hi) + ".png";
  function cardMainImg(hex) {
    if (!hex) return "";
    var src = hex.cardImg || HEXAGRAM_IMG[hex.num] || "";
    return src
      ? '<img class="card-main-img" src="' +
          src +
          '" alt="' +
          esc(hex.symbolLabel || "") +
          '">'
      : mainElIcon(hex);
  } /* [A] 易經核心：hexFromTitle / HEX_TABLE / changedHex / resolveInterpretation ==== */
  function hexFromTitle(title) {
    if (!title) return null;
    var m = String(title).match(/^(\d+)\s*(.*)$/);
    if (!m) return null;
    for (
      var num = parseInt(m[1], 10), _H = getHexagrams(), i = 0;
      i < _H.length;
      i++
    )
  ...
  function triKey(t2) {
    for (var k in TRI)
      if (TRI[k][0] === t2[0] && TRI[k][1] === t2[1] && TRI[k][2] === t2[2])
        return k;
    return "";
  }
  ...
  function saveUsers(u) {
    try {
      localStorage.setItem(USERS_KEY, JSON.stringify(u)),
        localStorage.setItem(FB_USERS_KEY, JSON.stringify(u));
    } catch (e) {}
  ...
  function resolveInterpretation(res) {
    ...
    return (
      h.num === 1 &&
        h.yongJiuText &&
        (special = { label: "用九", text: h.yongJiuText }),
      h.num === 2 &&
        h.yongLiuText &&
        (special = { label: "用六", text: h.yongLiuText }),
      {
        mode: "zhiGua",
        lineData: null,
        zhiGua: zhi6,
...
之卦` +
    ...
    if (r.mode === "zhiGua") {
      var zlabel = r.zhiGua
          ? r.zhiGua.num + " " + r.zhiGua.symbolLabel + "："
          : "",
...
` +
  ...
  function readingFocusText(res, cat) {
    ...
    if (
      ...
      r.lineData &&
      r.lineData.focus &&
      r.lineData.focus[cat]
    )
      return r.lineData.focus[cat];
    var h = res.hex;
    return (
      h.focus[cat] || h.focus["其他/不確定"] || h.focus.未選 || h.core || ""
    );
  }
  function readingGuideText(res) {
    var r = resolveInterpretation(res);
    if (r.mode === "line" && r.lineData && r.lineData.guide)
      return r.lineData.guide;
    var h = res.hex;
  ...
  var screens =
    document.querySelectorAll(
      ".screen",
    ); /* [C] 畫面切換：go / resetForm / setupChips / updateNav / doLogout ==== */
  function go(name) {
  ...
  function updateNav() {
    ...
    var dn = $("dash-name");
    dn &&
      (dn.textContent = isLoggedIn()
        ? t("dash.helloPre") + memberName() + t("dash.helloPost")
        : t("dash.helloGuest"));
  }
  ...
  $("btn-draw").addEventListener("click", function () {
    if (!drawFlipped) {
      ...
      del && (del.innerHTML = mainElIcon(res.hex)),
        (state.result = res),
        (state.saved = !1),
  ...
  } /* [E] 進階卜卦：tossOneLine / castSixLines / pickResult / startDivination / renderCarousel ==== */
  ...
  function pickResult() {
    ...
    function triToName(three) {
      for (var key = "", k = 0; k < 3; k++)
        key += three[k] === 7 || three[k] === 9 ? "1" : "0";
      for (var t2 in TRI) if (TRI[t2].join("") === key) return t2;
      return "";
    }
  ...
  function renderCarousel(keepPos) {
    if (state.result) {
      var _rc = $("result-carousel"),
        _prev =
          (keepPos && _rc && Math.round(_rc.scrollLeft / _rc.clientWidth)) || 0,
        res = state.result,
        h = res.hex,
      ...
      var slides = "",
        changedTxt =
          changed.length === 0 ? "" : "動爻：第 " + changed.join("、") + " 爻",
      ...
      (slides +=
        ...
        "</div>" +
        (changedTxt
          ? '<div class="changed-line">' + esc(changedTxt) + "</div>"
          : "") +
        "</div>"),
        (slides +=
      ...
      _c &&
        ((_c.scrollLeft = keepPos ? _prev * _c.clientWidth : 0), updateDots());
    }
  }
  function updateDots() {
    for (
  ...
  function saveFlow() {
    if (state.result) {
      var freshEls =
        state.result.method === "daily"
          ? unlockElementsFromHex(state.result.hex)
          : [];
      isLoggedIn()
        ...
        freshEls.length &&
          ...
          setTimeout(function () {
            toast(
              "✨ 元素任務完成：" + freshEls.join("、") + "（已解鎖收藏相簿）",
            );
          }, 1800));
    }
  }
  var authMode =
    "login"; /* [F] 帳號系統：setAuthMode / doLogin / 密碼找回 / settings ==== */
    ...
    if (authMode === "register") {
      ...
      (users[email] = { pass, name: nm, createdAt: Date.now() }),
        ...
        typeof FB != "undefined" &&
          ...
          FB.signUp(email, pass).then(function (r) {
            r &&
              r.error === "auth/email-already-in-use" &&
              toast("訳號已存在，請直接登入");
          });
    } else {
      var rec = users[email];
      if (typeof FB != "undefined" && FB && FB.signIn) {
        FB.signIn(email, pass).then(function (r) {
          ...
          (rec = users[email] || rec),
            rec || (rec = { pass, name: email.split("@")[0] || "會員" }),
  ...
  $("btn-forgot").addEventListener("click", function () {
    ...
    $("btn-forgot-find").addEventListener("click", function () {
      ...
      if (res) {
        ...
        if (!isValidEmail(em)) {
          res.innerHTML =
            '<span class="fr-err">Email 格式有誤（例：you@example.com）</span>';
          return;
        }
        if (typeof FB != "undefined" && FB && FB.resetPassword) {
          FB.resetPassword(em).then(function (r) {
            if (r && r.ok)
              ...
              var code = (r && r.error) || "";
              res.innerHTML =
                '<span class="fr-err">寄送失敗：' + esc(code) + "</span>";
            }
          });
          return;
        }
  ...
  function updateSettingAccount() {
    ...
    a &&
      (a.textContent = isLoggedIn()
        ? memberName() + t("setting.accountLogged")
        : t("setting.accountGuest"));
  ...
  var CAL_WEEK = ["日", "一", "二", "三", "四", "五", "六"],
    DIARY_KEY_PREFIX =
      "xingua_diary_"; /* [G] 日記系統：seedDiary / loadDiary / saveDiary / renderCalendar ==== */
  function seedDiary() {
    return [];
  }
  function loadDiary() {
    ...
    if (typeof FB != "undefined" && FB && FB.loadDiary) {
      var uid2 = uid2();
      FB.loadDiary(uid2).then(function (remote) {
  ...
  function mergeDraftsOnLogin() {
    try {
      ...
      for (var diary = loadDiary(), i = drafts.length - 1; i >= 0; i--) {
        ...
        !d ||
          !d.hex ||
          (diary.unshift(recordFromResult(d)),
          d.method === "daily" && unlockElementsFromHex(d.hex));
      }
    ...
    if (box) {
      for (var html = "", mi = 0; mi < 14; mi++) {
        ...
        for (var day = 1; day <= days; day++) {
          ...
          var recHex = dailyRec
              ? dailyRec.hex || hexFromTitle(dailyRec.title || "")
              : null,
          ...
          dailyRec && (recIcons ? (cls += " has-mood") : (cls += " has-hex")),
            ...
            dailyRec &&
              recIcons &&
              (html += '<span class="cal-mood">' + recIcons + "</span>"),
            (html += "</div>");
        }
        html += "</div></div>";
      }
  ...
  var COLLECT_BOOKS = [
    ...
    COLLECT_KEY =
      "xingua_collect_elements"; /* [H] 收藏守護：collectKey / renderCollect / unlockElementsFromHex / openGuardian ==== */
  function collectKey() {
  ...
  function saveCollectedElements(arr) {
    ...
    typeof FB != "undefined" &&
      FB &&
      FB.saveCollect &&
      FB.saveCollect(uid(), arr);
  }
  function unlockElementsFromHex(hex) {
    if (!hex) return [];
    for (
      var names = [hex.upperName, hex.lowerName],
        cur = getCollectedElements(),
        fresh = [],
        i = 0;
      i < names.length;
      i++
  ...
  function migrateGuestCollection() {
    try {
      ...
      for (var cur = getCollectedElements(), i = 0; i < guest.length; i++)
        cur.indexOf(guest[i]) === -1 && cur.push(guest[i]);
      saveCollectedElements(cur),
  ...
  function renderCollect() {
    ...
    if (body) {
      for (
        var arr = loadDiary(), divCount = 0, feedbackCount = 0, di = 0;
        di < arr.length;
        di++
      )
      ...
      for (
        var html = "",
          unlockedCount = 0,
          collected = getCollectedElements(),
          b = 0;
  ...
  function renderDetailCarousel(rec, keepPos) {
    ...
    if (c) {
      var _pidx =
          (keepPos && c && Math.round(c.scrollLeft / c.clientWidth)) || 0,
        h = rec.hex,
        chg = changedHex(rec),
        catTxt = rec.cat || "未選",
        changed = rec.changedLines || [],
        changedTxt = changed.length
          ? "動爻：第 " + changed.join("、") + " 爻"
          : "",
      ...
      if (
        ...
        _dc && (_dc.scrollLeft = keepPos ? _pidx * _dc.clientWidth : 0),
          (function () {
            for (var _dd = d.children, _i = 0; _i < _dd.length; _i++)
              _dd[_i].className =
                _i === Math.round(c.scrollLeft / c.clientWidth) ? "on" : "";
          })();
      }
    }
  }
  function openDetail(rec) {
    ...
    if (rec.type === "card") {
      if (
        ...
        var hCard = rec.hex || hexFromTitle(rec.title || ""),
          coreCard =
            hCard && hCard.core ? hCard.core : rec.note || rec.title || "",
    ...
    if (
      (dd &&
        (dd.textContent =
          rec.date + " · " + (rec.cat || "") + " · " + (rec.len || "")),
      detailNoteDraft &&
      detailNoteDraft.ts === rec.ts &&
      detailNoteDraft.title === rec.title
      ...
      rec.verify === "y" &&
        ((verify = "y"), $("verify-y").classList.add("on-y")),
      rec.verify === "x" &&
        ((verify = "x"), $("verify-x").classList.add("on-x")),
      rec.hex)
    )
      renderDetailCarousel(rec, !1);
    else if (rec.title) {
      var c = $("detail-carousel");
      c &&
        (c.innerHTML =
  ...
  function renderDiaryData() {
    for (var i = 0, dc = 0; i < diaryData.length; i++)
      diaryData[i].type === "divination" && dc++;
    var sc = $("stat-count");
    sc && (sc.textContent = dc);
    var sp = $("stat-pending");
    if (sp) {
      for (var n = 0, i = 0; i < diaryData.length; i++)
        diaryData[i].type === "divination" && !diaryData[i].verify && n++;
      sp.textContent = n;
    }
  }
  ...
  function saveDetailField(field, val) {
    if (detailRec) {
      ...
      diaryData.indexOf(detailRec) < 0 && diaryData.unshift(detailRec),
        saveDiary(diaryData);
    }
  }
  var noteSaved = !1;
...
`),
      (txt +=
        "情境解讀：關於" + res.cat + "：" + res.focus[res.cat] ||
        res.focus.未選 +
          `
...
` +
  ...
  function onBoot() {
    clearLegacySession(),
      ...
      typeof FB != "undefined" &&
        FB &&
        FB.loadUsers &&
        FB.loadUsers().then(function (remote) {
          if (remote)
            try {
  ...
  function initVisitCounter() {
    try {
      ...
      localStorage.setItem("dec12_visits", String(n));
      window.__visitCount = n;
    } catch (e) {
  ...
  function renderVisits() {
    ...
    el.textContent = txt;
  } /* [B] 語言系統：I18N / t / setLang / toggleLang / applyLangTexts ==== */
  var I18N = {
    zh: {
      nav: { login: "登入/註冊" },
      lang: { zh: "中文", en: "EN" },
      ...
      p2c: { save: "儲存到日記", redo: "重新起卦", share: "分享" },
      about: {
        title: "關於 DEC. 12",
        subtitle: "一個陪你靜下心、面對當下的手機卜卦小站",
        p1: "當感到迷茫時，內心的聲音被外界的喧囂淹沒時。我們需要的不是宿命論，而是一面溫柔映照內心、指引方向的鏡子。",
        p2: "卜卦罐的原型使用《易經》智慧，這是一套動態平衡的古老生命哲學。在這裡，每一次提問都是一次與自己、與宇宙節奏的深刻對話。讓這份智慧為你撥開迷霧，給予當下前行的勇氣。",
        p3: "",
        version: "DEC. 12 · 正式版 v27",
      },
      draw: {
        front: "每日靈感",
        hint: "每日靈感卡，點擊翻開。",
      ...
      milk: {
        ...
        r3: "愛心捐款",
        support: "請開發者一杯奶茶",
        note: "金額與流向公開透明，每月更新一次",
        charity:
          "DEC.12 是半公益平台：扣除營運成本後的淨利潤，一半將捐給公益團體",
      },
      p5: {
        title: "登入",
      ...
      share: {
        title: "分享",
        sub: "寄 email 給自己＝備份到信箱；與「儲存日記」不同，不會寫入日記",
        or: "或寄到 Email",
        send: "寄送",
      },
      save: { done: "已儲存" },
    ...
    en: {
      ...
      menu: {
        ...
        guardianFront1:
          "Quiet your heart and flip the first card of your journey\nA welcome blessing for every user",
      ...
      p2a: {
        ...
        title: "Star Jar",
        subtitle:
          "Close your eyes, pick a category, and let your thoughts focus under the stars.",
      ...
      p2c: { save: "Save to Diary", redo: "Redo", share: "Share" },
      about: {
        title: "About DEC. 12",
        subtitle:
          "A pocket divination site that helps you slow down and face the present",
      ...
      p2b: { casting: "Divining…" },
      milk: {
        title: "Milk Tea Fund",
        subtitle:
          "Every cup of milk tea is fuel for the developer to keep writing and maintaining",
        cups: "Total Cups",
        total: "Total Amount",
        ...
        charity:
          "DEC.12 is a semi-nonprofit platform: half of net profit after operating costs goes to charity",
      },
      p5: {
        title: "Log in",
  ...
  function detectLang() {
    ...
    try {
      ...
      if (
        nl.indexOf("zh") === 0 ||
        tz.indexOf("asia/taipei") === 0 ||
        tz.indexOf("asia/shanghai") === 0 ||
        tz.indexOf("asia/hong_kong") === 0 ||
        tz.indexOf("asia/macau") === 0 ||
        tz.indexOf("asia/singapore") === 0
      )
        return "zh";
    } catch (e) {}
    return "en";
  ...
  menuLangToggle &&
    menuLangToggle.addEventListener("change", function () {
